...
pub fn build(b: *std.Build) !void {
    ...
    if (target.result.os.tag == .windows) {
        ...
        //lib.root_module.addCMacro("_WIN32", ""); compiler complains that its redefined
...
// btw every entry here should have a reason
const unsupported_formats = [_][]const u8{
    "C4D", // fails to build, MSVC only
    "USD", // fails to build because it needs TinyUSDZ
    "VRML", // never seen this file format also needs to be cloned
    "X3D", // never seen this file format also needs to be cloned
};
